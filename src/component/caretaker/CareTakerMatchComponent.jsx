...
const CareTakerMatchComponent = ({ isOpen, onClose, no, takerMatchedListFn }) => {
    const [data, setData] = useState(init);

    useEffect(() => {
        takerMatchedListFn(no).then((data) => {
            console.log(data);
            setData(data);
        });
    }, [takerMatchedListFn, no]);
                ...
                <div className="bg-white rounded-lg shadow-lg overflow-hidden max-w-lg w-full">
                    ...
                    <table className="min-w-full leading-normal border border-gray-300 rounded-lg">
                        ...
                        <tr className="text-sm font-semibold text-left uppercase tracking-wide">
                            <th className="px-5 py-3">사용자(환자)</th>
                            <th className="px-5 py-3">간병인</th>
                        ...
                        {data.list.length === 0 ? (
                            ...
                            data.list.map((item) => (
                                <tr key={item.ctname + item.cgname}
...
export default CareTakerMatchComponent;
